...
#include "AnimNodePlugin_Mirror.h"
...
FAnimNodePlugin_Mirror::FAnimNodePlugin_Mirror()
...
void FAnimNodePlugin_Mirror::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FAnimNodePlugin_Mirror::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
...
bool FAnimNodePlugin_Mirror::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	for (auto& b_m : SetOfBonesToModify) {
...
void FAnimNodePlugin_Mirror::InitializeBoneReferences(const FBoneContainer& RequiredBones)
{
	for (auto It = BonesTransfroms.Map_IdxTransform.CreateConstIterator(); It; ++It) {
